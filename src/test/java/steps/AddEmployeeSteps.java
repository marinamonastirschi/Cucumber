...
public class AddEmployeeSteps extends CommonMethods {
    ...
    public void user_enters_firstname_and_middlename_and_verify_employee_has_been_added
            (io.cucumber.datatable.DataTable dataTable) {
        // wee need list of maps to get multiple values from datatable which is coming from feature file
        ...
        for (Map<String, String> employee : employeeNames) {
            ...
            // filling the names in the fields
...
//after adding one employee, we will add another employee
            // for this we are clicking on addEmployee button in the loop
            click(dashboardPage.addEmployeeButton);
        }
    }

    @When("user enters enters multiple employees using excel from {string} and verify it")
    public void userEntersEntersMultipleEmployeesUsingExcelFromAndVerifyIt(String sheetName) throws InterruptedException {
...
//it will check whether we have new element/value or not
            while (itr.hasNext()) {
                ...
                sendText(mapNewEmp.get("username"), addEmployeePage.userNameField);
                sendText(mapNewEmp.get("password"), addEmployeePage.passwordField);
